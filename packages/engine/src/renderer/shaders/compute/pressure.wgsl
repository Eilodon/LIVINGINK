...
@group(0) @binding(2) var divergenceInput: texture_2d<f32>; // B field
@group(0) @binding(3) var pressureOutput: texture_storage_2d<rgba32float, write>;
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let bC = textureLoad(divergenceInput, id, 0).x;

    let pNew = (L + R + B + T + params.alpha * bC) * params.beta; // Beta is 1/4 usually inv beta
    
    textureStore(pressureOutput, id, vec4<f32>(pNew, 0.0, 0.0, 1.0));
}
